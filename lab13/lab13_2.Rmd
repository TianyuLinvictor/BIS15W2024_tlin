...
bbox_sharks_fatal <- make_bbox(long_sharks_fatal, lat_sharks_fatal, f = 0.03)
```

```{r}
map_sharks_fatal <- get_stadiamap(bbox_sharks_fatal, maptype = "stamen_terrain", zoom=7)
```

```{r}
ggmap(map_sharks_fatal) + 
  geom_point(data = sharks_fatal, aes(longitude, latitude), size=0.4) +
  labs(x= "Longitude", y= "Latitude", title="Fatalities Locations")
```
