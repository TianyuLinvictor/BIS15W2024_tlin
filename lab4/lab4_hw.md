...
```r
homerange <- read.csv("data/Tamburelloetal_HomeRangeDatabase.csv", header=T)
```
...
```r
homerange_new <- clean_names(homerange)
names(homerange_new )
```
...
```r
table(homerange_new$trophic_guild)
```
...
```r
carnivores <-filter(homerange_new,trophic_guild=="carnivore")

herbivores <-filter(homerange_new,trophic_guild=="herbivore")
```
...
```r
owls<-filter(homerange_new,order=="strigiformes")
owls<-select(owls,"common_name","mean_mass_g","log10_mass" ,"family","genus","species")
```

The passerinum is the smallest owl. The common name is 	
Eurasian pygmy owl.It is the smallest owl in Europe. It is a dark reddish to greyish-brown, with spotted sides and half of a white ring around the back of the neck.[passerinum ](https://en.wikipedia.org/wiki/Eurasian_pygmy_owl). 
...
```r
bird_homerange <- filter(homerange_new,class=="aves")
```

Cheriway has the largest homerange. It is found throughout Central and South America but has been found in northern Minnesota to Tierra del Fuego. 
